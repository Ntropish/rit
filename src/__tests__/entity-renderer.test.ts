...
describe('Entity-tree renderer', () => {
  ...
  describe('let nodes', () => {
    it('binds expression result as a variable for children', async () => {
      const store = new ReactiveStore();
      await store.hset('user:1', 'name', 'Alice');
      await store.hset('user:1', 'role', 'admin');
      await setComponent(store, 'app', 'root');
      await setNode(store, 'app', 'root', {
        type: 'let',
        variable: 'user',
      ...
      await setNode(store, 'app', 'name', {
        type: 'expression',
        expr: 'user.name',
      });
      await setNode(store, 'app', 'role', {
        type: 'expression',
        expr: 'user.role',
      });
      ...
      expect(container.textContent).toBe('Aliceadmin');
      dispose();
    });

    it('reactively updates when bound expression changes', async () => {
      const store = new ReactiveStore();
      ...
      await setNode(store, 'app', 'display', {
        type: 'expression',
        expr: 'val',
      });
    ...
    it('preserves parent scope for children', async () => {
      const store = new ReactiveStore();
      await store.set('greeting', 'hello');
      await setComponent(store, 'app', 'root');
      await setNode(store, 'app', 'root', {
    ...
    it('supports object values with property access', async () => {
      const store = new ReactiveStore();
      await setComponent(store, 'app', 'root');
      await setNode(store, 'app', 'root', {
        type: 'let',
        variable: 'data',
        expr: '({ status: "success", items: [1, 2, 3] })',
        children: 'status,count',
      });
      await setNode(store, 'app', 'status', {
        type: 'expression',
        expr: 'data.status',
      });
      await setNode(store, 'app', 'count', {
        type: 'expression',
      ...
      expect(container.textContent).toBe('success3');
      dispose();
    });
  });
