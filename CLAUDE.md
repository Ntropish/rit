...
A versioned key-value store combining Redis semantics with Git-like version control, built on content-addressed prolly trees. Every mutation produces a new immutable tree with structural sharing — only affected chunks are rewritten.
...
1. **Store** (`src/store/`) — Content-addressed block store interface (hash → bytes). `MemoryStore` is the current implementation; the interface is designed for pluggable backends.
